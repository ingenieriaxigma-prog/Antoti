/**
 * 🎓 STATISTICS TOUR - Tour para Pantalla de Estadísticas
 */
...
const tourSteps: TourStep[] = [
  {
    id: 'welcome',
    target: '',
    title: '¡Bienvenido a Estadísticas! 📊',
    description: 'Analiza tus finanzas visualmente',
    icon: <Sparkles className="w-5 h-5" />,
    placement: 'center',
  },
  {
    id: 'month-selector',
    target: 'stats-month-selector',
    title: 'Selecciona el período 📅',
    description: 'Cambia entre meses para ver estadísticas históricas',
  ...
  },
  {
    ...
    description: 'Ve tus ingresos, gastos y balance del período',
    icon: <TrendingUp className="w-5 h-5" />,
    placement: 'bottom',
  },
  {
    id: 'pie-chart',
    target: 'stats-category-chart',
    title: 'Distribución por categoría 📊',
    description: 'Visualiza en qué gastas más dinero',
    icon: <PieChart className="w-5 h-5" />,
    placement: 'top',
  },
  {
    id: 'category-list',
    target: 'stats-category-list',
    title: 'Detalle de categorías 📋',
    description: 'Lista completa con porcentajes y montos',
    icon: <List className="w-5 h-5" />,
    placement: 'top',
  },
  {
    id: 'trend-chart',
    target: 'stats-trend-chart',
    title: 'Tendencia mensual 📈',
    description: 'Compara tus gastos e ingresos mes a mes',
    icon: <BarChart3 className="w-5 h-5" />,
    placement: 'top', // 🔧 CAMBIO: De 'bottom' a 'top' para que el tooltip aparezca arriba
  },
];

const TOUR_STORAGE_KEY = 'oti_statistics_tour_completed';

export function StatisticsTour({ onComplete }: StatisticsTourProps) {
  ...
  useEffect(() => {
    ...
    const updateHighlight = () => {
      ...
      if (element) {
        ...
        // 🔧 MEJORA: Para el paso de tendencia mensual, hacer MUCHO más scroll
        if (step.id === 'trend-chart') {
          // Primero hacer scroll al elemento hacia el inicio
          element.scrollIntoView({ behavior: 'smooth', block: 'start', inline: 'nearest' });
          
          // Hacer scroll agresivo en múltiples etapas para asegurar visibilidad completa
          setTimeout(() => {
            const scrollContainer = document.querySelector('.overflow-y-auto');
            if (scrollContainer) {
              const currentScrollTop = scrollContainer.scrollTop;
              // PRIMER SCROLL: 400px adicionales
          ...
          setTimeout(() => {
            ...
            if (scrollContainer) {
              const currentScrollTop = scrollContainer.scrollTop;
              // TERCER SCROLL: 150px más para asegurar
              scrollContainer.scrollTo({
                top: currentScrollTop + 150,
                behavior: 'smooth'
              });
            }
          }, 750);
        } else {
          // Para otros pasos, usar comportamiento normal
  ...
  }, [currentStep, step.target, step.id, isActive]);
        ...
        >
          {highlightRect && !isCenterStep ? (
            ...
            <svg className="absolute inset-0 w-full h-full">
              <defs>
                <mask id="spotlight-mask-statistics">
              ...
              <rect 
                ...
                height="100%" 
                fill="rgba(0, 0, 0, 0.7)"
                mask="url(#spotlight-mask-statistics)"
              />
            </svg>
          ) : (
...
export function resetStatisticsTour() {
  localStorage.removeItem(TOUR_STORAGE_KEY);
}

export function shouldShowStatisticsTour(): boolean {
